...
def read_twits(data_file: set, users: tuple) -> dict:
    logger = prefect.context.get("logger")
    ru_users, uk_users = users
        ...
        try:
            for line in tqdm(f.readlines()):
                ...
                for tweet in data["data"]:
                    if tweet["author_id"] in ru_users:
                        ru_texts.append(tweet["text"])
                    elif tweet["author_id"] in uk_users:
                        uk_texts.append(tweet["text"])

        except Exception as e:
            logger.error(f"Error reading file {data_file}, error: {e}")

    pickle.dump(uk_texts, (HOME / os.environ.get("RESULTS_PREFIX") / f"{data_file}-uk.pkl").open("wb"))
    pickle.dump(ru_texts, (HOME / os.environ.get("RESULTS_PREFIX") / f"{data_file}-ru.pkl").open("wb"))
