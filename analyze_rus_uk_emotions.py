...
def collect_files(s3_path: str = "twits/") -> list:
    # TODO: uk|ru pattern
    return list(BASE_DIR.glob("*/*.jsonl.gz-uk.pkl"))


@task(name="Clean corpus", checkpoint=True, result=results, target="{file}_ru_uk_only_clean_text.df")
...
@task(name="Analyze Texts", checkpoint=True, result=results, target="{file}_ru_uk_only_emotions.df")
